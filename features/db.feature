Feature: Test tasks for namespace 'db'
	...
	Scenario: Check if settings upload is possible with settings given as arguments
		When I successfully run `cap dev "db:upload_settings[dkdeploy-core_db_1,3306,dkdeploy_core,root,ilikerandompasswords,utf8]"`
	...
	Scenario: Check if settings upload is possible with settings given as enviroment variables
		When I successfully run `cap dev db:upload_settings DB_HOST=dkdeploy-core_db_1 DB_PORT=3306 DB_NAME=dkdeploy_core DB_USERNAME=root DB_PASSWORD=ilikerandompasswords DB_CHARSET=utf8`
		Then a remote file named "shared_path/config/db_settings.dev.yaml" should exist
